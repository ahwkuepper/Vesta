...
public final class BridgeTransport: NSObject, LightTransport, @unchecked Sendable {
    ...
    /// The address is stored data, not a literal, so building a URL from it can
    /// fail — a stored value containing a space used to crash the app at launch.
    /// Article 4: decoding device input must be total.
    /// A CLIP v2 resource URL. Throws rather than force-unwrapping: every call
    /// site is already throwing, and an unusable stored address should surface as
    /// an error the user sees rather than a crash at launch.
    func resource(_ path: String) throws -> URL {
        guard let url = endpoint("clip/v2/resource/" + path) else {
            throw BridgeError.malformedAddress(credentials.address)
        }
        return url
    }
    ...
    func endpoint(_ path: String) -> URL? {
        ...
        components.host = credentials.address
    ...
    public func watchRawEvents(seconds: Double, onLine: @escaping @Sendable (String) -> Void) async {
        guard let url = endpoint("eventstream/clip/v2") else { return }
    ...
    public func rawResource(_ type: String) async throws -> String {
        let (data, _) = try await send(request(try resource(type)))
    ...
    func loadLights(attempts: Int) async throws -> [Light] {
        let (data, response) = try await send(request(try resource("light")),
                                              attempts: attempts)
    ...
    private func put(_ body: [String: Any], to id: Light.ID) async throws {
        let url = try resource("light/\(id.uuidString.lowercased())")
    ...
    -> (URLSession.AuthChallengeDisposition, URLCredential?) {
        ...
        if current.publicKeyHash == nil,
           let hash = BridgePinning.publicKeyHash(of: leaf) {
            state.withLock {
                $0.publicKeyHash = hash
                // Credentials from before salting have none; give them one now
                // rather than fingerprinting with an empty salt forever.
                if $0.fingerprintSalt == nil { $0.fingerprintSalt = Fingerprint.newSalt() }
            }
