...
///
/// Lumo sends no telemetry — nothing leaves the machine on its own, and that stays
...
enum Diagnostics {
    ...
    static func report(store: LightStore, mode: AppModel.Mode) -> String {
        ...
        lines.append("Lumo diagnostics — \(Self.timestamp())")
        lines.append("")
        ...
        if let credentials = BridgeStore.load() {
            ...
            lines.append("bridge id  \(Self.fingerprint(credentials.bridgeID))")
            lines.append("app key    \(Self.fingerprint(credentials.appKey))")
        } else {
    ...
    /// Identifies a value without disclosing it: same value, same fingerprint.
    private static func fingerprint(_ key: String) -> String {
        var hash: UInt64 = 5381
        for byte in key.utf8 { hash = (hash &* 33) &+ UInt64(byte) }
        return String(format: "%04x (%d chars)", UInt16(truncatingIfNeeded: hash), key.count)
    }
