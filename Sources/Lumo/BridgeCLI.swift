...
enum BridgeCLI {
    ...
    static func testScenes() async -> Int32 {
        ...
        do {
            ...
            for scene in existing {
                ...
                say("  \(scene.name) — \(room)\(scene.isEditable ? " (Lumo)" : " (Hue app)")")
            }

            guard let room = rooms.first else { say("\nno rooms to test with"); return 1 }
            say("\ncreating a scene in \(room.name)…")
            let created = try await transport.createScene(named: "Lumo Test Scene", in: room.id)
    ...
    static func makePresets(room roomName: String) async -> Int32 {
        ...
        do {
            ...
            for preset in presets {
                ...
                // every run — but only ones Lumo created.
    ...
    /// Reports how Lumo would find the bridge again, for diagnosing recovery.
