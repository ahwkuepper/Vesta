...
struct MenuBarView: View {
    ...
    private var header: some View {
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text("Lumo").font(.system(size: 13, weight: .semibold))
                Text(summary)
                    .font(.system(size: 11))
    ...
    private var pairingHint: some View {
        VStack(alignment: .leading, spacing: 5) {
            ...
            Text("Lumo can see these bulbs over Bluetooth but can’t command them. "
    ...
    private var settingsMenu: some View {
        Menu {
            ...
            Button("Quit Lumo") { NSApplication.shared.terminate(nil) }
                .keyboardShortcut("q")
        } label: {
