...
/// app and any other controller — Vesta reads them rather than inventing its own
...
/// every light, including effects like gradients that Vesta does not model.
public struct RoomScene: Identifiable, Sendable, Equatable {
    ...
    /// Set when Vesta created it, so the UI can offer deletion without risking a
