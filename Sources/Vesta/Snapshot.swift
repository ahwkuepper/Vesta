...
enum Snapshot {
    ...
    static func renderAll(to directory: URL) async throws {
        ...
        if canCaptureCompositedFrames,
           ProcessInfo.processInfo.environment["VESTA_SNAPSHOT_BACKDROP"] == "desktop" {
            desktopBackdrop = await captureDesktop(on: NSScreen.main ?? NSScreen.screens[0])
            if desktopBackdrop == nil {
                FileHandle.standardError.write(
                    Data("could not photograph the desktop; using the neutral backdrop\n".utf8))
            }
        }
        GlassSettings.isEnabled = canCaptureCompositedFrames
            && ProcessInfo.processInfo.environment["VESTA_SNAPSHOT_GLASS"] != "0"
    ...
    private static func render(_ view: some View, appearance: NSAppearance.Name,
        ...
        do {
            ...
            guard let fallback = hosting.bitmapImageRepForCachingDisplay(in: hosting.bounds) else {
                FileHandle.standardError.write(
                    Data("no bitmap for \(url.lastPathComponent)\n".utf8))
                return
            }
            ...
            warnOnce(error)
            FileHandle.standardError.write(
                Data("  \(url.lastPathComponent): drawn in-process, Liquid Glass MISSING\n".utf8))
        }

        guard let png = bitmap.representation(using: .png, properties: [:]) else {
            FileHandle.standardError.write(Data("no png for \(url.lastPathComponent)\n".utf8))
            return
        }
        ...
        do {
            ...
            FileHandle.standardError.write(
                Data("FAILED \(url.path): \(error.localizedDescription)\n".utf8))
        }
    }
