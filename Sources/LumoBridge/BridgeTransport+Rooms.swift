...
///
/// Both are read from the bridge rather than invented locally, so Lumo, the Hue app
...
extension BridgeTransport {
    ...
    /// Marks scenes Lumo created, so they can be safely offered for deletion.
    static let appTag = "lumo"
    ...
    public func createScene(named name: String, in room: Room.ID,
        ...
        let (responseData, response) = try await send(
            request(base.appendingPathComponent("scene"), method: "POST", body: data))
    ...
    public func deleteScene(_ id: RoomScene.ID) async throws {
        let url = base.appendingPathComponent("scene/\(id.uuidString.lowercased())")
    ...
    private func rawData(for type: String) async throws -> Data {
        let (data, response) = try await send(request(base.appendingPathComponent(type)))
    ...
    func put(_ body: [String: Any], to path: String) async throws {
        let data = try JSONSerialization.data(withJSONObject: body)
        let (responseData, response) = try await send(
            request(base.appendingPathComponent(path), method: "PUT", body: data))
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
            throw BridgeError.http((response as? HTTPURLResponse)?.statusCode ?? -1,
